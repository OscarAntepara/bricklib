  ...
  if (cart != MPI_COMM_NULL) {
    ...
    auto brick_func = [&]() -> void {
      ...
      while (resMG > 1e-10 && niterMG_brick < max_num_iter){
        ...
        bElem h_res[1];
        h_res[0]=0.0;
        hipMalloc(&d_res,sizeof(bElem));
        hipMemcpy(d_res, h_res, sizeof(bElem), hipMemcpyHostToDevice);

        maxNorm_brick(mgb,d_res);

        hipMemcpy(h_res, d_res, sizeof(bElem), hipMemcpyDeviceToHost);
    ...
    for (int ilevel = 0; ilevel < num_levels; ilevel++)
    {
      ...
      hipFree(mgb.levels[ilevel]->bInfo_dev);
      hipFree(mgb.levels[ilevel]->dd_dev_ptr);
    }
    free(mgb.levels);
