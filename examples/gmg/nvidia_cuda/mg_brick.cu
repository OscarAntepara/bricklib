                   ...
                   double& timer)
{
...
#ifndef GPU_AWARE
  {
    ...
    cudaMemcpy(field.dat.get() + field.step * bDecomp->sep_pos[0],
...
#endif
    t_a = omp_get_wtime();
    cudaMemcpy(field_dev.dat.get() + field.step * bDecomp->sep_pos[1],
...
void init_mg_brick(MG_brick& mg, int num_levels, MPI_Comm cart,int* coo, std::vector<unsigned>& dom_size, int amr_levels){
  ...
  for (int ilevel = 0; ilevel < num_levels; ilevel++)
  {
    ...
    auto _bInfo_dev = movBrickInfo((*mg.levels[ilevel]->bInfo), cudaMemcpyHostToDevice);
    {
      unsigned size = sizeof(BrickInfo<3>);
      cudaMalloc(&mg.levels[ilevel]->bInfo_dev, size);
      cudaMemcpy(mg.levels[ilevel]->bInfo_dev, &_bInfo_dev, size, cudaMemcpyHostToDevice);
    }

    mg.levels[ilevel]->bStorageX_dev = new BrickStorage (movBrickStorage((*mg.levels[ilevel]->bStorageX), cudaMemcpyHostToDevice));
    mg.levels[ilevel]->bStorageAx_dev = new BrickStorage (movBrickStorage((*mg.levels[ilevel]->bStorageAx), cudaMemcpyHostToDevice));
    mg.levels[ilevel]->bStorageRhs_dev = new BrickStorage (movBrickStorage((*mg.levels[ilevel]->bStorageRhs), cudaMemcpyHostToDevice));
    mg.levels[ilevel]->bStorageRes_dev = new BrickStorage (movBrickStorage((*mg.levels[ilevel]->bStorageRes), cudaMemcpyHostToDevice));
    ...
    cudaMalloc((void **) &mg.levels[ilevel]->dd_dev_ptr, ngBricksBytes);
    int ngBricks_acum = 0;
    for (int i = 0; i < bDecomp->ghost.size(); ++i) {
    ...
    cudaMemcpy(mg.levels[ilevel]->dd_dev_ptr, mg.levels[ilevel]->dd_ptr, ngBricksBytes, cudaMemcpyHostToDevice);
    ...
    cudaMalloc((void**)&mg.levels[ilevel]->dom_len_dev,2*sizeof(bElem));
    cudaMemcpy(mg.levels[ilevel]->dom_len_dev,dom_len_ptr,2*sizeof(bElem),cudaMemcpyHostToDevice);
