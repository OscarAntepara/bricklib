  ...
  for (long jb = gb; jb < strideb[1] - gb; ++jb) {
    for (long j = 0; j < TILE; ++j) {
      ...
      for (long ib = gb; ib < strideb[0] - gb; ++ib) {
        unsigned b = grid[(strideb[2]/2-1)][jb][ib];
...
int main(int argc, char **argv) {
  ...
  if (cart != MPI_COMM_NULL) {
    ...
    if (rank == 0) {
      std::cout << "Running with MPI_GPU_AWARE = " << GPU_AWARE << std::endl;
    }
    int cnt;
    double total;
    ...
    auto brick_func = [&]() -> void {
      ...
      while (resMG > 1e-10 && niterMG_brick < max_num_iter){
        ...
        bElem *d_res;
        bElem h_res[1];
        cudaMallocManaged((void**)&d_res, sizeof(bElem), cudaMemAttachGlobal);
        cudaMemsetAsync(d_res, 0, sizeof(bElem));
        ...
        mgb.levels[0]->timers.maxNormRes += elapsed / 1000.0;
    ...
    for (int ilevel = 0; ilevel < num_levels; ilevel++)
    {
      ...
      free(mgb.levels[ilevel]->dd_ptr);
      cudaFree(mgb.levels[ilevel]->bInfo_dev);
      cudaFree(mgb.levels[ilevel]->dd_dev_ptr);
    }
    free(mgb.levels);
    ...
    for (int ilevel = 0; ilevel < num_levels; ilevel++)
    {
    ((MEMFD *)(*mgb.levels[ilevel]->bStorageX).mmap_info)->cleanup();
    ((MEMFD *)(*mgb.levels[ilevel]->bStorageOut).mmap_info)->cleanup();
    ((MEMFD *)(*mgb.levels[ilevel]->bStorageRhs).mmap_info)->cleanup();
    ((MEMFD *)(*mgb.levels[ilevel]->bStorageRes).mmap_info)->cleanup();
    }
#endif
  }
